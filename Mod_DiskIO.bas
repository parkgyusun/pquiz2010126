...
Public Function GetFileDate(FileName As String, FDate As Integer, FTime As Integer) As Boolean
    ...
    If hFile <> 0 Then
        ...
        'With SysTime
            FD = (SysTime.wYear - 1980) * 2 ^ 9
            FD = FD + (SysTime.wMonth * 2 ^ 5)
            FD = FD + SysTime.wDay
            FT = SysTime.wHour * 2 ^ 11
            FT = FT + (SysTime.wMinute * 2 ^ 5)
            FT = FT + SysTime.wSecond
        'End With
    End If
...
Public Function SetFileDate(FileName As String, dCreated As Date, dModified As Date, dAccessed As Date) As Boolean
    ...
    'With SysTime                           'Convert Dates to FILETIMEs
        SysTime.wYear = Year(dCreated)
        SysTime.wMonth = Month(dCreated)
        SysTime.wDay = Day(dCreated)
        SysTime.wHour = Hour(dCreated)
        SysTime.wMinute = Minute(dCreated)
        SysTime.wSecond = Second(dCreated)
    'End With
    SystemTimeToFileTime SysTime, fCreated

    'With SysTime
        SysTime.wYear = Year(dModified)
        SysTime.wMonth = Month(dModified)
        SysTime.wDay = Day(dModified)
        SysTime.wHour = Hour(dModified)
        SysTime.wMinute = Minute(dModified)
        SysTime.wSecond = Second(dModified)
    'End With
    SystemTimeToFileTime SysTime, fModified

    'With SysTime
        SysTime.wYear = Year(dAccessed)
        SysTime.wMonth = Month(dAccessed)
        SysTime.wDay = Day(dAccessed)
        SysTime.wHour = Hour(dAccessed)
        SysTime.wMinute = Minute(dAccessed)
        SysTime.wSecond = Second(dAccessed)
    'End With
...
Public Function GetZipDate(FDate As Integer, FTime As Integer) As Date
    ...
    'With SysTime
    GetZipDate = CDate(Format$(SysTime.wMonth) & "/" & _
                       Format$(SysTime.wDay) & "/" & _
                       Format$(SysTime.wYear) & " " & _
                       Format$(SysTime.wHour) & ":" & _
...
errorOut:
    MsgBox "Error While Attempting to Create Directories on Destination Drive.", vbExclamation, "SETUP"
    ChDir ThisDir
    CreatePath = False
End Function
