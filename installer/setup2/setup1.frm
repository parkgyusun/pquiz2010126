...
Begin VB.Form frmSetup1 
   ...
   Begin VB.Label lblDDE 
      AutoSize        =   -1  'True
      BorderStyle     =   1  '단일 고정
      Caption         =   "이 레이블은 프로그램 관리자로의 DDE 연결에 사용됩니다."
      BeginProperty Font 
         Name            =   "굴림"
         Size            =   9
         Charset         =   129
...
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "frmSetup1.Form.Load", Command$
#End If
...
#End If
    ...
    ' that directory.
    '
    gstrSetupInfoFile = gstrSrcPath & gstrFILE_SETUP
    ...
    Dim pos As Integer
    pos = InStrRev(gsTEMPDIR, "\")
    gsTEMPDIR = Left(gsTEMPDIR, pos)
    AddDirSep gsTEMPDIR
...
ExitSetup:
    ...
    MsgBox "다음으로 진행되는 MySql ODBC를 설치 하시기 바랍니다.", vbExclamation
    
'    Clipboard.Clear
'    Clipboard.SetText getPerfectFile(gstrDestDir, "MyODBC-standard-3.51.9-win.exe")
    
    Call Shell(getPerfectFile(gstrDestDir, "MyODBC-standard-3.51.9-win.exe"), vbNormalFocus)
