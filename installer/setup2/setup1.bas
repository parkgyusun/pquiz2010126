...
#If PGSLOG Then
Public Const gstrPGSLOGFILENM = "c:\pquiz.log"
#End If
'
' Icon Information
'
...
Public Sub CopySection(ByVal strSection As String)
    ...
    intIdx = 1
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "CopySection.strMultDirBaseName", strMultDirBaseName
#End If

#If PGSLOG Then
    logput gstrPGSLOGFILENM, "CopySection.gsTEMPDIR Exist Check", gsTEMPDIR
#End If
...
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "CopySection sFile.strSrcName=", sFile.strSrcName
#End If
...
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "CopySection strNewSrc=", strNewSrc
#End If

#If PGSLOG Then
    logput gstrPGSLOGFILENM, "압축해제1 gsCABFULLNAME", gsCABFULLNAME
#End If

#If PGSLOG Then
    logput gstrPGSLOGFILENM, "압축해제2 sFile.strSrcName", sFile.strSrcName
#End If

#If PGSLOG Then
    logput gstrPGSLOGFILENM, "압축해제3 strNewSrc", strNewSrc
#End If

#If PGSLOG Then
    logput gstrPGSLOGFILENM, "압축해제4 gintCabs ", CStr(gintCabs)
#End If

#If PGSLOG Then
    logput gstrPGSLOGFILENM, "압축해제5  gstrSrcPath ", gstrSrcPath
#End If

        ExtractFileFromCab gsCABFULLNAME, sFile.strSrcName, strNewSrc, gintCabs, gstrSrcPath
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "압축해제완료", ""
#End If

        If FileExists(strNewSrc) Then
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "●FileExists(" & """" & strNewSrc & """" & ") OK", "OK"
#End If
            sFile.strSrcName = gsTEMPDIR & sFile.strDestName
            sFile.intDiskNum = gintCurrentDisk
        End If
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "sFile.strSrcName=", sFile.strSrcName
#End If
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "sFile.intDiskNum=", CStr(sFile.intDiskNum)
#End If
        '
...
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "strSrcName 파일존재여부체크", strSrcName
#End If
        ...
        Else
            ...
            '
#If PGSLOG Then
    logput gstrPGSLOGFILENM, "◆ Can't find the file.", strSrcName
#End If
            ...
            End If
            strMsg = ResolveResString(resCOMMON_CANTFINDSRCFILE, gstrPIPE1, strSrcName) & "[" & GetWinPlatform() & "]"
                ...
                Select Case SourceFileIsNewer(sFile, strSrcName, strSrcDir, strDestName, strDestDir, strDestVer)
                    ...
                    Case fcOlder
                        If sFile.fDestDirRecognizedBySetupExe And FileInUse(strDestDir & strDestName) Then
                            ...
                            owValue = owYes
                        Else
'                        '설치하려고 하는것이 옛날것일때 어찌 할것인지를 묻는 내용인데 무시하도록 한다.
'
'                            '
...
#End If
    ...
    strSrcPath = Replace(strSrcPath, """", "")
    If fErr Then GoTo BadCommandLine

    strAppRemovalLog = strExtractFilenameArg(strCommand, fErr)
    strAppRemovalLog = Replace(strAppRemovalLog, """", "")
    If fErr Then GoTo BadCommandLine

    strAppRemovalEXE = strExtractFilenameArg(strCommand, fErr)
    strAppRemovalEXE = Replace(strAppRemovalEXE, """", "")
    If fErr Then GoTo BadCommandLine
        ...
        Select Case intDrvType
            ...
            Case intDRIVE_FIXED
                ...
                Else
                    strDetectPath = gstrSrcPath
                End If
                strMsg = ResolveResString(resCOMMON_CANTFINDSRCFILE, gstrPIPE1, strDetectPath & gstrSEP_DIR & strDetectFile) & "[" & GetWinPlatform() & "]"
        End Select
...
AskWhatToDo:
                    ...
                    '아래 라인 변경 파일 등록시 오류 발생은 자동 무시 한다. 20060406
                    Select Case vbIgnore 'MsgError(strMsg, vbExclamation Or vbAbortRetryIgnore, gstrTitle)
                        Case vbAbort:
                            ExitSetup frmSetup1, gintRET_ABORT
                            GoTo AskWhatToDo
                        Case vbRetry:
...
Private Function strExtractFilenameArg(str As String, fErr As Boolean)
    ...
    If inn = 1 Then
        For i = LBound(arr) To UBound(arr) - 2
            strExtractFilenameArg = strExtractFilenameArg & arr(i) & " "
        Next
        
        strExtractFilenameArg = Trim$(strExtractFilenameArg)
        fErr = False
        Exit Function
    ElseIf inn = 2 Then
        strExtractFilenameArg = Trim$(arr(UBound(arr) - 1))
        fErr = False
        Exit Function
    ElseIf inn = 3 Then
        strExtractFilenameArg = Trim$(arr(UBound(arr) - 0))
        fErr = False
        Exit Function
